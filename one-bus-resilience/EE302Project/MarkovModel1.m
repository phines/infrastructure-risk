function [Statematrix,t] = MarkovModel1(N,edges,Hurricane,neighborWeight,maxtimesteps,startingstate,recoveryRate)
...
while t < 4 || ((sum(statematrix(t,:)) ~=0)  &&  t<=maxtimesteps)
    ...
    for n = 1:N
        ...
        if ~state(n)
            ...
            if sum(stress) == 0
                probrecover = 1-((1-recoveryRate)*(1-neighborWeight) + neighborWeight*(neighborsdown));
            else 
                probrecover = 0;
            end

...
function crisp_rlopf_island(ps,Pd_max,ps_islands)
    ...
    for z in 1:M
        ### collect the data that we will need ###
        # bus data
        n = size(ps.bus[ps_islands[z].bus,:],1) # the number of buses
        bi = sparse(ps.bus[ps_islands[z].bus,:id],fill(1,n),collect(1:n)) # helps us to find things
        # load data
        nd = size(ps.shunt[ps_islands[z].shunt,:],1)
        D = bi[ps.shunt[ps_islands[z].shunt,:bus]]

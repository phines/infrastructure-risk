...
function crisp_Restore(ps,l_recovery_times,g_recovery_times,dt,time,t0;load_cost=0)
    # constants
    tolerance = 10^(-6);
    if sum(load_cost)==0
    ...
    Restore = DataFrame(time = time, load_shed = load_shed, perc_load_served = perc_load_served,
    ...
    for i = 1:length(Time)
        ...
        ps.branch.status[time .>= l_recovery_times] .= 1;
        ps.gen.status[time .>= g_recovery_times] .= 1;
...
function crisp_mh_rlopf!(ps,dt,time)
    t = 1:dt:time;
    Ti = size(t,1);
    # constants
    tolerance = 1e-6
    ...
    if n>1
        ...
        @constraint(m, [k=1:Ti-1], E[:,k+1] .== (E[:,k] + dt.*(Ps[:,k]))) # storage energy at next time step
        @constraint(m, [k=1:Ti], 0 .<= (E[:,k] + (dt/60).*(Ps[:,k])) .<= E_max) # storage energy

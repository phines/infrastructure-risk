...
function crisp_mh_rlopf!(ps,dt,t_win)
    ...
    t_off = ps.gen[gst,:time_off]./dt ./ ps.baseMVA .* ps.gen[gst,:status]
    t_on = ps.gen[gst,:time_on]./dt ./ ps.baseMVA .* ps.gen[gst,:status]
    if any(G.<1) || any(G.>n)
    ...
    if n>1
        ...
        @constraint(m, [j=1:Ti], E[:,j] .== (E[:,j-1] + ((dt/60) .*(Ps[:,j])))) # storage energy at next time step
        ...
        @constraint(m, gon[:,0] .== gon1)
        @constraint(m, goff[:,0] .== goff1)

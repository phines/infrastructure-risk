...
Nlinesa0 = zeros(Nevents);
Nlinesp1 = zeros(Nevents);
Times = [];
for n = 1:Nevents
    Nlines[n] = init_out_zipf_a0(s_line,maxLinesOut,TotalLines);
end
for n = 1:Nevents
    Nlinesa0[n] = init_out_zipf_a0(s_line,maxLinesOut,TotalLines);
    while Nlinesa0[n]==0
        Nlinesa0[n] = init_out_zipf_a0(s_line,maxLinesOut,TotalLines);
    end
end
for n = 1:Nevents
    Nlinesp1[n] = init_out_zipf_p1(s_line,maxLinesOut,TotalLines);
end
for n = 1:Nevents
    push!(Times,RecoveryTimes(mu_line,sigma_line,1)[1,1]);
end
Nline = sort(Nlines);
Nlinea0 = sort(Nlinesa0);
Nlinep1 = sort(Nlinesp1);
ccdf_nlines_empir = collect(length(Nline):-1:1)./length(Nline);
Times = Times[:];
RecT = sort(Times);
ccdf_rectime_empir = collect(length(RecT):-1:1)./length(RecT);

num = DataFrame(No_Lines_Out_AO = Nline, No_Lines_Out_NAO = Nlinea0, No_Lines_Out_P1 = Nlinep1, CCDF = ccdf_nlines_empir);
CSV.write("results\\checks\\LineOutageRealiz2_v1-3.csv",num);
...
CSV.write("results\\checks\\LinesDist_v3.csv",NumLinesAnalytic);
T = 0:0.1:10000;

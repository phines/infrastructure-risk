...
#PBS -o /users/m/k/mkellygo/CRISP/output1/${PBS_JOBNAME}_${PBS_JOBID}.out
...
ti=$(date +"%H:%M:%S.%N")
echo "started job at $ti" ## $PASS part $PASSB at $ti"
cd /users/m/k/mkellygo/CRISP
module load julia
## times and runs program
time julia scripts/run_CRISP_Rdist_movh_vacc.jl ${PBS_ARRAYID}
## ${PBS_ARRAYID} $PASSB
ti=$(date +"%H:%M:%S.%N")
echo "completed job at ti" ## $PASS part $PASSB at $ti"

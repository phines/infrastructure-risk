...
function Resilience(N,ps_folder,out_folder,dt;param_file = "")
    #constants
    debug=1;
    tolerance1 = 10^(-4);
    ...
    ps.storage = ps.storage[ps.storage.E .!= 0,:]
    crisp_dcpf_g_s!(ps)
    ...
    for iterat in 1:Num
        ...
        for i in 1:M
            ...
            crisp_dcpf_g_s!(psi);
            # run lsopf
            dti = 10; # first minute, affects ramp rate limits - rateB
            crisp_lsopf_g_s!(psi,dti);
...
include("src/CRISP_RLOPF_mh_varLoad.jl")
